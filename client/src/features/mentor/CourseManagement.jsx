...
const CourseManagement = () => {
  ...
  useEffect(() => {
    // Simulate fetching courses taught by the mentor
    const fetchCourses = async () => {
      try {
        const dummyCourses = [
          { id: 1, title: 'Beginner Web Development', description: 'Dasar-dasar HTML, CSS, JavaScript.', level: 'Beginner', isPublished: true },
          { id: 2, title: 'Intermediate JavaScript', description: 'Konsep lanjutan JavaScript dan React.', level: 'Intermediate', isPublished: false },
        ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (form.id) {
        ...
        await new Promise(resolve => setTimeout(resolve, 500));
        setCourses(courses.map(course => (course.id === form.id ? { ...form } : course)));
        alert('Kursus berhasil diperbarui!');
      } else {
        // Simulate API call to create course
        await new Promise(resolve => setTimeout(resolve, 500));
        const newCourse = { ...form, id: Date.now() }; // Assign a temporary ID
  ...
  return (
    <div className="container mx-auto py-10">
      <h1 className="text-3xl font-bold mb-6">Manajemen Kursus & Materi</h1>
      ...
      <div className="bg-white p-8 rounded-lg shadow-md">
        <h2 className="text-2xl font-semibold mb-4">Daftar Kursus Anda</h2>
        {courses.length > 0 ? (
                ...
                {courses.map(course => (
                  <tr key={course.id}>
                    ...
                    <td className="py-2 px-4 border-b">
                      ...
                      <Link
                        to={`/mentor/courses/${course.id}/materials`}
          ...
          <p className="text-gray-500">Anda belum memiliki kursus. Tambahkan yang baru!</p>
        )}
      </div>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white p-8 rounded-lg shadow-lg w-full max-w-md">
            <h2 className="text-2xl font-bold mb-4">{currentCourse ? 'Edit Kursus' : 'Tambah Kursus Baru'}</h2>
